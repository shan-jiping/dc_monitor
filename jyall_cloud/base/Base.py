...
class dockerM(dockerBase):
    def __init__(self,session,version='1.20'):
    ...
    def monitor_agent(self):
        ...
        agent_list=self.Session.query(agent.agent_ip,agent.port,monitor.monitor_id).join(monitor).filter(monitor.type==10,monitor.status==1).all()
    ...
    def BaseTemplate(self):
        ...
        '''
        cpu=template(template_id=1,type='CPU Used Percent',items='Total,%',threshold=80)
        iowait=template(template_id=2,type='IOWait Percent',items='Total,%',threshold=20)
        nett=template(template_id=3,type='Net Traffic',items='Total,KB',threshold=102400)
        netr=template(template_id=4,type='Net Route',items='route')
        mem=template(template_id=5,type='MEM Used Percent',items='Total,%',threshold=80)
        diskUage=template(template_id=6,type='DISK Used Percent',items='df -h',threshold=80)
        IO=template(template_id=7,type='IO usage',items='Total,MB',threshold=100)
        proc=template(template_id=8,type='process',items='ps -aux')
        port=template(template_id=9,type='port',items='netstat -nlptu')
        agent=template(template_id=10,type='agent',items='u',threshold=0)
    ...
    def defineMonitor(self,ip,containid,type1,userid,domain1,threshold1,email1='',interval1=15,items1='',status1=1,dockerid=''):
        
        result=[]
        agentid=self.Session.query(agent.agent_id).filter(agent.agent_ip==ip).first()[0]
        monitorid_list=self.Session.query(monitor.type).join(agent).filter(agent.agent_ip==ip,monitor.container_id==containid).all()
        #print agentid
        monitorresult=[]
        for ii in monitorid_list:
            monitorresult.append(int(ii[0]))
        #print monitorresult
        
        i=int(type1)
        #print i
        #print i in monitorresult
        if i in monitorresult:
            ##update
            Xquery=self.Session.query(monitor).filter(monitor.type==i,monitor.container_id==containid,monitor.agent_id==agentid)
...
class dockerAgent(dockerBase):
    ...
    def insertSYS(self,agentid,containid):
        ...
        for item in monitor_list:
            ...
            if item.type=='3':
                if Session.query(history_net).filter(history_net.monitor_id==item.monitor_id).first()==None:
                ...
                histio=history_io(monitor_id=item.monitor_id,
                                   read=data['io']['read'],
                                   write=data['io']['write'],
                                   total=data['io']['total']
                                   )
...
def initParam(monparam,Session):
    param={}
    templateresult=Session.query(template).filter(template.template_id!=10).all()
    for i in templateresult:
        param[int(i.template_id)]={'threshold':i.threshold,'status':0}
    
    
    #w11='1:80:1$2:20:1$3:102400:1$4::1$5:80:1$8:sshd:1$9:22:1'
    mon_param={}
    for i in  monparam.split('$'):
        type1,threshold1,status=i.split(':')
        mon_param[int(type1)]={u'threshold':threshold1,u'status':int(status)}
    
    result=dict(param.items()+mon_param.items())
    return result
